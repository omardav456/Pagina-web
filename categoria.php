...
$categoria = isset($_GET['categoria']) ? $_GET['categoria'] : 'No hay categoría seleccionada';

// Aquí puedes usar la variable $categoria para mostrar datos relevantes
echo "<h1>Categoría Seleccionada: " . htmlspecialchars($categoria) . "</h1>";

// Agrega aquí el código para mostrar los datos de la categoría
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div>
  <div class="title-container">
    <h1 class="title">Productos <?php echo $_GET['categoria']?></h1>
  </div>
  </div>
</div>
<?php
  ...
  $stmt = $conn->prepare("SELECT Id_Producto, Nombre_Producto, Descripcion_Producto, Cantidad_Producto, Precio_Producto, URL_Producto FROM Producto WHERE Categoria_Producto = ?");
    
  // Bind del parámetro
  $stmt->bind_param("s", $categoria); // Cambia "s" a "i" si 'categoria' es un entero
